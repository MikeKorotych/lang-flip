...
final class OllamaAssistant: AIAssistant {
    ...
    func rewriteSentence(_ input: AIRewriteRequest, completion: @escaping (AIRewriteResult) -> Void) {
        ...
        let system = TextCorrectionPrompt.system(language: lang, allowLayoutRepair: false)
        runInference(
            system: system,
            prompt: input.text,
    ...
    func fixSelection(_ input: AIFixRequest, completion: @escaping (AIFixResult) -> Void) {
        ...
        let system = TextCorrectionPrompt.system(language: lang, allowLayoutRepair: true)
        runInference(
            system: system,
            prompt: input.text,
            options: ["temperature": 0.1, "num_ctx": 2048, "num_predict": 256]
    ...
    func warmUp() {
        ...
        let system = TextCorrectionPrompt.system(language: "input language", allowLayoutRepair: true)
        runInference(
            system: system,
